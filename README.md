...
| **Kick** | Square wave | Pitch-sweep sine, soft | Square + sine sub, heavy | Square + sine sub, maximum amplitude | Sine thump 130–145 Hz (tombak dom) | Pitch-sweep sine, very loud | Square + sine sub, tight | Sine thump + noise transient (tabor) | Sine thump (taiko) | Sine thump, very soft | Pitch-sweep sine, beats 2+4 (surdo) |
...
**Melody.** Two square waves form a power chord — root and perfect fifth (7 semitones). Both go through a very fast 1 ms attack and a high 92 % sustain envelope, then the combined signal is **hard-clipped** at ±0.65 and rescaled to ±1. Hard clipping (as opposed to the soft `x / (1 + |x|)` saturation used in rock) squares off the peaks completely, adding the dense harmonic cloud that distinguishes a truly distorted guitar tone. A quieter harmony voice is layered underneath with the same treatment.

**Bass.** A square wave driven hard (amplitude × 2.8) through a soft saturator `d / (1 + |d|)`, resulting in a thick, compressed bass tone with significant upper harmonics. The envelope has a very fast 1 ms attack, 18 ms decay to an 88 % sustain — always present, never sustained loosely.

**Voice.** The default vibrato sine is replaced with a harsher vocal: faster vibrato (7 Hz), wider pitch depth (±4 %), then the result is pushed through the same soft saturator at gain × 3.2. This produces the clipped, harmonic-rich character of metal harsh vocals without generating noise.

**Drums.** The kit is the loudest of all modes, designed for maximum impact:
- *Kick* — a square wave at 55–63 Hz (higher than rock, punchier) layered with a 42 Hz sine sub. Both are enveloped tightly (40–55 ms decay) for a sharp double-kick attack at 70–38 % amplitude respectively. Patterns are drawn from three dedicated tables: 8th-note double-kick (every two 16th steps), gallop (1+&1+2), or syncopated 16ths — the specific pattern is seeded so each file gets a consistent feel.
- *Snare* — an explosive noise burst at up to 52 % amplitude on the backbeats (steps 4 and 12), with a 220 Hz sine crack layered on accented positions. Non-backbeat hits (ghost notes in some snare patterns) drop to 22 % amplitude.
- *Hi-hat* — crisp noise bursts at 22 % amplitude, very short (under 14 ms on closed steps). Three hat patterns — 8th notes, relentless 16ths, or syncopated 16ths — driven by the seed. An open hat on beat 3 or &4 (seed-dependent) gives a brief ring between sections.
